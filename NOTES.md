
# NOTES
## Helpful links 

#### need to add `export NODE_OPTIONS=--openssl-legacy-provider` to ~./zshrc to get webpacker-dev-server working
* https://stackoverflow.com/questions/69692842/error-message-error0308010cdigital-envelope-routinesunsupported
 

#### store JSON in table
* https://stackoverflow.com/questions/51008503/how-to-store-json-to-database-in-rails
* https://edgeguides.rubyonrails.org/active_record_postgresql.html#json-and-jsonb


#### ActiveModel::Error
* https://api.rubyonrails.org/v6.1.3/classes/ActiveModel/Error.html


#### API Endpoint for geocoding an address
* https://stackoverflow.com/questions/98449/how-to-convert-an-address-to-a-latitude-longitude
* https://stackoverflow.com/questions/31811074/openstreetmap-get-coordinates-from-address
* https://nominatim.openstreetmap.org/search?q={address_here}&format=json&polygon=1&addressdetails=1


#### USPS API
* https://www.usps.com/business/web-tools-apis/address-information-api.htm
* https://www.usps.com/business/web-tools-apis/documentation-updates.htm
* https://www.usps.com/business/web-tools-apis/welcome.htm


#### API Endpoint for weather
* https://weather-gov.github.io/api/
* https://weather-gov.github.io/api/general-faqs
* https://www.weather.gov/documentation/services-web-api
* https://api.weather.gov/points/44.9092,-93.2957
  * use lat & long coordinates to get the MPX weather station
  * from ['properties']['forecast'] or ['forecastHourly]


* https://stackoverflow.com/questions/1252210/parametrized-get-request-in-ruby

* https://www.cyberciti.biz/tips/finding-bash-perl-python-portably-using-env.html

#### Testing private methods in RSpec
* https://stackoverflow.com/questions/16599256/testing-private-method-in-ruby-rspec


#### Exclude from RuboCop
* https://stackoverflow.com/questions/36896279/how-to-tell-rubocop-to-ignore-a-specific-directory-or-file

#### Overcommit gem
* https://github.com/sds/overcommit


#### Misc
* https://stackoverflow.com/questions/25568541/class-hierarchy-in-ruby
* https://www.rubyguides.com/2019/02/ruby-io/
* https://thoughtbot.com/blog/rubys-argf
* https://stackoverflow.com/questions/14824551/rendering-json-in-controller
* https://www.bigbinary.com/learn-rubyonrails-book/setting-up-rake-tasks
* https://docs.github.com/en/github/importing-your-projects-to-github/importing-source-code-to-github/adding-an-existing-project-to-github-using-the-command-line
* https://unix.stackexchange.com/questions/29608/why-is-it-better-to-use-usr-bin-env-name-instead-of-path-to-name-as-my
* https://github.com/travisjeffery/timecop
* https://stackoverflow.com/questions/8706930/converting-nested-hash-keys-from-camelcase-to-snake-case-in-ruby


#### Markdown Syntax
* https://www.markdownguide.org/cheat-sheet/


#### XSS Prevention
* https://www.stackhawk.com/blog/rails-xss-examples-and-prevention/


#### Form ever follows function  (i.e. function before form)
* https://www.heartinternet.uk/blog/form-over-function-beautifully-designed-things-that-simply-dont-work/


