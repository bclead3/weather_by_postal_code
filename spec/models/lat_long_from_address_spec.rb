...
RSpec.describe LatLongFromAddress, type: :model do
  ...
  let(:valid_attributes) do
    { address: '821 Marquette Avenue', city: 'Minneapolis', state: 'MN', zip: '55401' }
  end
  ...
  describe 'instantiation' do
    it 'works with valid attributes' do
      ...
      expect(lat_long_from_address.zip).to eq(55_401)
    end
  ...
  describe '#populate' do
    ...
    it 'works on minimal attributes' do
      ...
      expect(lat_long_from_address.source_place_id).to eq(324_648_326)
      expect(lat_long_from_address.zip).to eq(55_401)
      expect(lat_long_from_address.postal_code).to eq('55401')
      expect(lat_long_from_address.previously_looked_up).to be_truthy
      expect(lat_long_from_address.json_resp.keys).to eq(%w[place_id licence osm_type osm_id boundingbox lat lon
